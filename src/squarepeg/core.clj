...
Example:
  ...
  (fn [input bindings context memo]
    (loop [val [] input input bindings bindings memo memo]
      (let [r (rule input bindings context memo)]
        (if (success? r)
          (recur (into val (:s r)) (:i r) (:b r) (:m r))
...
(defn mkmsg
  ...
  (fn [input bindings context memo]
    (let [r (rule input bindings context memo)]
      (if (failure? r)
        (fail (str msg " Reason: " (:fail r)) (:memo r))
        r))))
...
(defmacro defrule
  "Set up a rule in the namespace (using `def`). Will wrap it with a
  scope, memoization, and a failure message that uses the name."
  [name body]
  (assert (symbol? name))
  `(do
     (declare ~@(externs body))
     (def ~(with-meta name
             {:arglists ''([input] [input context] [input bindings context memo])
              :doc (str name " is a squarepeg parser rule. Call with a seq of input or use it as a rule (4 arguments).")})
       (mkfn (mkmsg (mkscope (mkmemo ~body))
                    ~(str "Failed trying to match '" name "'"))))))
...
(defrule end (mkmsg (mknot anything) "Expected end of input."))
